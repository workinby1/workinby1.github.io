...
export  function replaceEnds(arr:number[], startValue:number[], endValue:number[]):number[]{
  ...
  export  function cube(num:number):number{
   ...
   export  function square(arr:number[]|number):number[]|number{
    ...
    // }
     ...
     export function absVal(arr:number[]|number):number[]|number{
      if(Array.isArray(arr)){
  ...
  export  function higho(func:(num:number[]|number)=>number[]|number, num:number[]|number):number[]|number{
      return func(num) 
    }
...
export function getAllSessionsForUser(dailyRecord: Day[], userId: number): Session[] {
  let allSessions: Session[] = getAllSessions(dailyRecord);
...
//   //Use an appropriate array method to get array of just duration times [60, 45, 30, 15 ,75] Hint:  use getAllSessions. 
 ...
 export const getAllDurations = function (dailyRecord:Day[]):number[]{
