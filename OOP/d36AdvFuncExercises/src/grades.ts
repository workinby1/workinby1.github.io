 ...
 * @returns {number} difference of the identifiers
 */
answerComparator:function(ans1:Answer, ans2:Answer) {
  //IMPLEMENT THIS
...
const sortedKey = this.key.slice().sort(this.answerComparator);
...
getAverage:function ():number {
  //IMPLEMENT THIS
  const totalScores = this.students.reduce((total, student)=> total +this.scoreStudent(student.sid),0)
